...
我们可以调用 [MxCADUiPrPoint](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUiPrPoint.html#class-mxcaduiprpoint) 方法获取导出范围。
...
在上述步骤中我们已经获取到了pdf在图纸上的范围，因此接下来我们可以直接调用 [McObject.saveFileToUrl()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McObject.html#savefiletourl) 方法来导出目标pdf。其中，如果不设置导出图纸范围则将默认导出所有的图纸内容。
...
:::tip
注意：
将图纸转为PDF需调用node.js后台服务，因此，在调用上述方法之前请先启动[云图开发包](https://www.mxdraw.com/download.html)中的后台服务。
...
export default () => {
    ...
    return (<div style="height: 630px; overflow: hidden;">
      ...
      id="myBtn" onClick={()=>Mx_Export_Pdf()}>导出pdf</button>
