...
为了方便后期管理与修改标注，我们可以通过继承 [McDbCustomEntity](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbCustomEntity.html) 自定义实体类来扩展实现自定义连续测量类。由于连续测量是计算圆弧与直线的组合长度，因此，我们可以将连续的直线与圆弧整合为 [McDbPolyline](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbPolyline.html) 多段线。其中，我们可以通过设置多段线的bulge凸度来绘制圆弧。

然后，我们可以利用 [McDbText](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbText.html) 构造测量信息文本对象，将对象组合长度的标注信息绘制在页面中。
...
```
* 我们调用 [MxCADUiPrPoint](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUiPrPoint.html#class-mxcaduiprpoint) 取点对象进行连续取点，并可以通过设置 [MxCADUiPrPoint.setKeyWords()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUiPrPoint.html#setkeywords) 关键字列表来控制操作过程中的回退到上一个捕捉点或结束点对象捕捉。其中，在捕捉弧线的过程中我们需要调用 [MxCADUtility.findEntAtPoint()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUtilityClass.html#findentatpoint) 方法来确定我们取点时选取的实体对象并调用 [MxCADResbuf](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADResbuf.html) 来过滤出目标弧线，然后调用 [MxCADUtility.calcBulge()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUtilityClass.html#calcbulge) 方法获取到圆弧的凸度。值得注意的是捕捉弧线时，我们需要先捕捉到弧线的某一个端点后再去捕捉整条弧线，以此来保证我们捕捉的线段时一条连续的线段。
...
由于之前我们封装的 [McDbTestAreaComment 自定义面积标注类](./2.测量面积（多边形、矩形）.md#功能实现) 底层逻辑也是将目标图形通过多段线重绘并计算面积，因此，我们可以根据上述封装的取点函数进行取点（包含弧线）来计算包含弧线的异形面积。
