...
为了方便后期管理与修改标注对象，我们可以通过继承 [McDbCustomEntity](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbCustomEntity.html) 自定义实体类来扩展实现局部放大类。我们根据用户框选的放大区域在图纸中调用 [MxCADSelectionSet.crossingSelect()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADSelectionSet.html#crossingselect) 方法获取到目标区域内的所有实体，再通过调用 [McDbBlockTableRecord.appendAcDbEntity()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbBlockTableRecord.html#appendacdbentity) 方法将区域内的实体整合为块，然后调用 [McDbSpatialFilter.setBoundary()](../../../../api/classes/2d.McDbSpatialFilter.md#setboundary) 方法根据自身需求对块的外边框做剪裁。最后根据设置的比例因子对块做缩放。
...
我们可以利用取点对象 [MxCADUiPrPoint()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUiPrPoint.html) 获取放大范围的中心点，再调用 [MxCADUiPrDist()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUiPrDist.html) 设置放大范围。
...
我们可以利用 [MxCADUiPrPoint](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUiPrPoint.html) 取点对象在页面中交互取点。在取点过程中，我们可以通过 [MxCADUiPrPoint.setUserDraw()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUiPrPoint.html#setuserdraw) 方法动态绘制对象，使用户更加直观的观察到放大标注的位置变化。
