...
为了方便后期管理与修改标注，我们可以通过继承 [McDbCustomEntity](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbCustomEntity.html) 自定义实体类来扩展实现面积标注类。其中，在 mxcad 中多边形与矩形都是通过多段线绘制出来的，多段线对象对应的实体类为 [McDbPolyline](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbPolyline.html)，该类提供了获取或设置图形对象相关信息的属性或方法，我们可以根据我们的功能需求去选择调用。在测量面积（多边形、矩形）功能中，我们可以调用 [McDbPolyline.getLength()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbPolyline.html#getlength) 方法获取多段线对象的长度、以及在多段线闭合的情况下调用 [McDbPolyline.getArea()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbPolyline.html#getarea) 方法获取面积。

然后，我们可以利用 [McDbMText](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbMText.html) 构造测量信息多文本对象，将对象的标注信息绘制在页面中。
...
测量多边形面积需要连续取点，且取点不少于3个。我们可以利用 [MxCADUiPrPoint](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUiPrPoint.html) 取点对象在图纸中循环取点来确定需要测量多边形的位置与大小，并通过 [MxCADUiPrPoint.setKeyWords()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUiPrPoint.html#setkeywords) 为其设置关键字列表，使用户能够实现自主撤销操作或提前结束操作，方便交互。
