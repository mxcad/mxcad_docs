...
我们可以利用 [MxCADUiPrPoint](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUiPrPoint.html) 取点对象在图纸中选取需要测量的目标点对象。
...
我们可以利用选择实体对象 [MxCADUiPrEntity()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUiPrEntity.html) 根据用户鼠标点击的坐标得到对应的实体，其中我们需要只选择直线对象，因此，我们再调用 [MxCADResbuf()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADResbuf.html) 为选择实体对象设置过滤器来过滤出目标实体。
...
在 mxcad 中直线对象对应的实体类为 [McDbLine](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbLine.html) ，该类提供了获取或设置直线相关信息的属性或方法，我们可以根据我们的功能需求去选择调用。在测量点到直线距离的功能中，我们可以调用 [McDbLine.getClosestPointTo()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbLine.html#getclosestpointto) 方法获取目标点对线到目标直线对象上的垂足点，该垂足点到目标点对象的距离就是点到直线的距离。最后我们再调用 [mxcad.drawDimAligned()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McObject.html#drawdimaligned) 绘制对齐标注。
