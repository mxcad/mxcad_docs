...
为了方便后期管理与修改标注，我们可以通过继承 [McDbCustomEntity](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbCustomEntity.html) 自定义实体类来扩展实现弧长标注类。其中，在 mxcad 中圆弧对象对应的实体类为 [McDbArc](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbArc.html) ，该类提供了获取或设置圆弧相关信息的属性或方法，我们可以根据我们的功能需求去选择调用。在测量弧长功能中，我们可以调用 [McDbArc.getLength()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbCircle.html#getlength) 方法获取圆弧长度。

然后，我们可以利用 [McDbText](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbText.html) 构造测量信息文本对象，将圆弧的标注信息绘制在页面中。
...
我们可以利用选择实体对象 [MxCADUiPrEntity()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUiPrEntity.html) 根据用户鼠标点击的坐标得到对应的实体，其中我们需要只选择圆弧对象，因此，我们再调用 [MxCADResbuf()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADResbuf.html) 为选择实体对象设置过滤器来过滤出目标实体。
