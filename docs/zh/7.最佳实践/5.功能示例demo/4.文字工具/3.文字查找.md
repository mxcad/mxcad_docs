...
我们可以调用 [MxCADUiPrString](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUiPrString.html#class-mxcaduiprstring) 设置我们需要查找的文字内容。此外，也可以通过输入框或其他方式获取到需要查找的文字内容。
...
我们可以调用 [MxCADUiPrKeyWord](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUiPrKeyWord.html#class-mxcaduiprkeyword) 根据用户输入关键字来设置文字查找的类型，如：全字匹配、文字包含、文字前缀匹配、文字后缀匹配等。
...
我们可以调用 [MxCADResbuf](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADResbuf.html#class-mxcadresbuf) 设置过滤器，再通过 [MxCADUtility.userSelect()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADSelectionSet.html#userselect) 方法过滤出选择实体中的单行文本对象。

由于在上述步骤中我们已经获取到了用户选择的查找方式，因此，我们将过滤出来的文字实体根据不同查找方式过滤出符合要求的对象，并将其通过 [McObject.addCurrentSelect()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McObject.html#addcurrentselect) 方法加入当前选择实体中便于我们观察。
