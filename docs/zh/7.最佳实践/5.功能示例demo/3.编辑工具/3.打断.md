...
下面我们将介绍如何利用 mxcad 插件实现在CAD图纸中的曲线打断功能。该功能中用户可以通过打断点将一条完整的曲线切割成两部分或多部分，这些切割出来的部分又将变成独立的曲线。其中，mxcad 中的曲线类实体包括：[McDbPolyline](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbPolyline.html) 多段线、[McDbArc](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbArc.html) 圆弧、[McDbCircle](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbCircle.html) 圆、[McDbEllipse](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbEllipse.html) 椭圆/弧、[McDbLine](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbLine.html) 直线。打断功能能够帮助用户对图纸中的目标曲线进行分段测量、删除、调整曲线长度等高效操作。

## 功能实现

1. 选中目标曲线

我们可以通过 [MxCADResbuf](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADResbuf.html#class-mxcadresbuf) 设置过滤器，再调用 [MxCADUiPrEntity](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUiPrEntity.html) 获取用户选择的实体，过滤器会使用户只能选择 [McDbCurve](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbCurve.html) 曲线类的实体方便我们后续的打断操作。为了方便我们更直观的观察到选中的实体是哪一个，我们可以将选中的实体通过 [McDbCurve.highlight()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbCurve.html) 方法来设置为高亮显示。
...
我们可以通过 [MxCADUiPrPoint](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUiPrPoint.html) 在上述步骤中选中的目标曲线上进行取点操作，这些被选中的点即作为目标曲线的断点。在绘制过程中，我们可以以断点为圆心动态绘制一个圆以此来方便我们观察断点的具体位置。
...
完成上述步骤后，我们将目标曲线根据这些断点调用 [McDbCurve.splitCurves()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbCurve.html#splitcurves) 方法进行曲线切割，然后我们就能够获取到打断后的曲线。
...
我们可以调用 [McDbCurve.IntersectWith()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbCurve.html#intersectwith) 方法获取到曲线与曲线之间的交点，并以这些交点作为曲线的打断点，对曲线进行打断。在下面的示例中，我们将曲线间的交点作为断点去对曲线进行切割打断，并绘制出一定长度的断口来方便观察。用户可根据自己的实际需求做相应调整。
