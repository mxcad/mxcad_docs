...
我们可以调用 [MxCADSelectionSet](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADSelectionSet.html#class-mxcadselectionset) 选择需要进行缩放的实体。根据选中的实体获得其包围盒的最大点与最小点，并根据获得的数据设置缩放的距离比例。
...
我们通过调用 [MxCADUiPrPoint](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUiPrPoint.html) 取点对象来确定缩放对象的基点，调用 [MxCADUiPrDist](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUiPrDist.html) 来动态设置或直接输入缩放对象的缩放因子。
...
由于我们获取缩放因子的方式有两种，一种是根据用户移动鼠标位置与与基点之间的动态距离来计算缩放因子，一种是用户直接通过输入框输入实体的缩放因子，因此，我们可以通过 [MxCADUiPrDist.getDetailedResult()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUiPrDist.html#getdetailedresult) 方法获取交互退出的详细原因来根据两种方式分别设置实体的缩放因子。缩放后，调用 [McObject.updateDisplay()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McObject.html#updatedisplay) 方法来更新图纸显示。
