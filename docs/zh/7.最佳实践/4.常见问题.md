...
解决方案2：将图纸上缺少的字体文件放入 mxcad 的字体文件加载路径`fontspath`下。
...
我们可以通过调用 CAD 图块引用实体 McDbBlockReference() 中的 [getAllAttribute()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbBlockReference.html#getallattribute) 方法得到块引用的所有属性文字对象id，然后通过对象id获取到图块中的实体详情。
```ts
...
在CAD中，图层允许用户将图形对象组织到不同的层中，以便于管理和编辑。每个图层都有其独特的特性，如颜色、线型、线宽等，这些特性决定了图层上对象的显示方式。我们可以通过设置 [MxCADResbuf](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADResbuf.html#class-mxcadresbuf) 过滤器来实现图层上的对象过滤。
...
1. 若只是在原有 MxCAD App 界面的基础上简单修改页面布局可参考 [MxCAD APP在线CAD 配制说明](https://www.mxdraw3d.com/mxcad_docs/zh/7.%E6%9C%80%E4%BD%B3%E5%AE%9E%E8%B7%B5/1.MxCAD%20APP%E5%BA%94%E7%94%A8%E9%9B%86%E6%88%90.html#mxcad-app在线cad-配制说明) ,根据自身需求修改 mxUiConfig.json 文件来配置界面。
...
在项目初始化时通过 [MxCpp.App.addNetworkLoadingTrueTypeFont()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McAppType.html#addnetworkloadingtruetypefont) 方法来设置我们系统需要加载哪些字体，方便后面我们修改字体时加载使用。
