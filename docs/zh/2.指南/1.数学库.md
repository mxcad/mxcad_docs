...
4. CAD坐标系, 是指CAD图纸的坐标系, 在mxcad中 [McGePoint3d](../../api/classes/McGePoint3d.md) 的坐标就是CAD图纸坐标

mxcad的坐标体系相互转换的方法是在[mxdraw 坐标转换](https://mxcadx.gitee.io/mxdraw_docs/coordinate/CoordinateTransformation.html)中全部列举出来了
...
在three.js中有向量`THREE.Vector3` 而对应的在mxcad也存在[McGeVector3d](../../api/classes/McGeVector3d.md)表示3D空间中的矢量(向量)
...
这是最常使用的一个类 [McGePoint3d](../../api/classes/McGePoint3d.md) 代表3D空间中的一个点 的构造类
...
在mxcad在中 [McGeMatrix3d](../../api/classes/McGeMatrix3d.md) 类表示3D空间的仿射变换
...
[MxCADResbuf](../../api/classes/MxCADResbuf.md)是CAD二次开发中`resbuf`(即 "result buffer") 用来传递数据的一种数据结构。

一般用于对象属性查询、自定义对象的定义和存储、XDATA（扩展数据）处理、绘图实体的编辑 等

例如在[MxCADSelectionSet](../../api/classes/MxCADSelectionSet.md)选择集中表示过滤对象:
...
MxCADUtility是[MxCADUtilityClass](../../api/classes/MxCADUtilityClass.md) 提供的实例, 提供了很多实用的方式。

[MxCADUtility.calcBulge](../../api/classes/MxCADUtilityClass.md#calcBulge) 计算圆弧凸度
