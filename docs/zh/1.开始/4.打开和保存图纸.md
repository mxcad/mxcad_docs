...
`mxweb文件`是 mxcad 支持打开的特定文件类型，传统的CAD图纸文件格式可通过云图开发包转换为`mxweb文件`，具体操作可参考[图纸转换](../1.开始/3.图纸转换.md)。

### 打开mxweb文件

mxcad实例对象首先传入的参数是mxweb文件的网络路径，即通过互联网可以直接访问到对应的`mxweb文件`。如果我们想在本地直接打开`mxweb文件`，可调用方法[openWebFile()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McObject.html#openwebfile) 直接打开本地`mxweb文件`。
```ts
...
<<<<<<< HEAD
示例demo参考：[导出jpg功能](../7.最佳实践/5.功能示例demo/5.其他/1.导出jpg.md)
...
点击 [McObject.saveFileToUrl()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McObject.html#savefiletourl) 查看方法详情。

<<<<<<< HEAD
示例demo参考：[导出pdf功能](../7.最佳实践/5.功能示例demo/5.其他/2.导出pdf.md)
...
默认情况下, 我们提供`saveFileToUrl` 来实现保存CAD图纸, 它实际上帮我们将当前的mxweb数据上传到了你指定的一个后端接口中。下面是它的使用方法:
>>>>>>> 336a3932e80ac24ad804265e40f4ad4d62d3927c
