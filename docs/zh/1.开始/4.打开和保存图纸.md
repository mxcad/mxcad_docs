...
mxcad实例对象首先传入的参数是mxweb文件的网络路径，即通过互联网可以直接访问到对应的`mxweb文件`。如果我们想在本地直接打开`mxweb文件`，可调用方法[openWebFile()](../../api/classes//McObject.md#openwebfile) 直接打开本地`mxweb文件`。
```ts
...
mxcad 中提供了实例方法 saveFile() 来保存编辑后的`mxweb文件`。其中，该方法提供了多个parameter参数，用户可根据自身需求设置对应参数值，具体参数设置可参考[saveFile() API文档](../../api/classes/McObject.md#savefile)。例如保存的mxweb文件不进行压缩(打开速度更快，文件体积变大)，可将 saveFile() 最后一个参数设置为`{ compression: 0 }` 。
