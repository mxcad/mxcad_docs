...
| Name | Type | Default value | Description |
| :------ | :------ | :------ | :------ |
| `call?` | (`MxCpp`: [`MxCppType`](../classes/2d.MxCppType.md)) => `void` | `undefined` | 回调函数，在加载完成后执行 |
| `locateFile` | `any` | `undefined` | 函数用于确定 wasm 文件的位置，这里提供了一个默认的值 |
| `wasmBinary?` | [`ArrayBuffer`]( https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/ArrayBuffer ) | `undefined` | wasm 二进制数据 |
| `fontspath?` | `string` | `undefined` | 字体文件的路径 |
| `networkFonts?` | `string`[] | `undefined` | 网络字体的数组 |
| `mxcad?` | `any` | `undefined` | - |
