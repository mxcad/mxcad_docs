...
你可以正常使用"mapbox-gl": "1.13.3" 版本的 api 继续对地图进行开发，mxcad 中绘制编辑图纸并不会影响它， 因为 mxcad 中绘制的 CAD 图纸始终放在一个 mapbox 提供的 webgl 层上。
...
最后安装`mapbox-gl@1.13.3`的依赖， 就可以在 ts 文件中直接引入它。
