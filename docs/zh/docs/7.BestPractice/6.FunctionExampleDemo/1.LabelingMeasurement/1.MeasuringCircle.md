...
下面我们将介绍如何利用 mxcad 插件实现在 CAD 图纸中测量圆的功能，该功能中用户点击目标圆对象将自动标记出这个圆的半径、面积值和周长值，同时用户可以自定义选择标注文字的位置。测量圆功能能够帮助用户快速掌握目标圆对象的数据信息，方便统计工程量。
...
class McDbTestMeasuringCircle extends McDbCustomEntity {
  ...
  public worldDraw(draw: MxCADWorldDraw): void {
    ...
    mText.contents = `半径：${radius.toFixed(2)} \\P 周长：${length.toFixed(
      2,
    )} \\P 面积：${area.toFixed(2)}`;
...
export default () => {
  ...
  mxcad.create({
    ...
    onOpenFileComplete: (mxcad) => {
      ...
      // 显示命令行消息变化
      MxFun.listenForCommandLineInput(({ msCmdTip, msCmdDisplay }) => {
  ...
  return (
    <div style="height: 600px; overflow: hidden;">
      ...
      <div style="position: relative; height: 450px; overflow: hidden;">
        <canvas
          id="myCanvas"
          style="position: absolute; inset: 0; display: block; width: 100% !important; height: 100% !important; max-width: 100% !important; max-height: 100% !important;"
        ></canvas>
      </div>
      <div style="width: 100%;height: 150px;margin:10px 0px">
        <textarea
          style="width: 100%;height: 100px;background-color: #000;color: #fff;border-radius: 5px"
          id="myArea"
          readonly="true"
        ></textarea>
      </div>
