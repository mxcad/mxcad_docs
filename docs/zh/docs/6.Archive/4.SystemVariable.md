...
线宽在CAD中可以看作是图形对象轮廓线条的宽度属性，用于控制绘制或显示对象时线条的粗细程度。开启线宽显示后，用户能够直观地区分不同对象的线宽差异，从而更清晰地表达图形的层次结构与设计意图，类似于在图纸上通过不同粗细的墨线来区分主要结构与辅助标注。线宽显示还有助于检查图纸中线宽设置的合理性，作为打印输出前视觉校验的参考依据。

我们可以通过获取系统变量 `LWDISPLAY` 的值来控制是否显示线宽，并结合对应方法更新显示状态。
