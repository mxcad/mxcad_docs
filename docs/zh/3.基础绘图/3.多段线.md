...
多段线是由弧和线组成的多段连接线。用户可以用任意线型绘制多段线。绘制了至少一条多段线后，可以调用 erase() 方法来删除先前的线段。当绘制两个或两个以上的多段线后，可以调用 isClosed() 方法闭合当前多条线段，绘制以第一条线段的起点为终点的线段。多段线还可以调用 addVertexAt() 方法设置多段线的顶点位置、线段宽度以及圆弧凸度等。结合[命令模式](../4.交互绘图/1.命令模式.md)，可实现添加顶点、删除顶点、转换为圆弧、转换为直线等完整功能。

点击 [McDbPolyline()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbPolyline.html)  查看详细属性和方法说明。
