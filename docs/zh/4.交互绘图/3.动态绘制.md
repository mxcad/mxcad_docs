...
我们可以直接从mxcad中导出以draw开头的函数, 每一个函数对应着一个动态绘制图形的功能。这些功能存在一些命令行交互(输入参数、关键词)，类似大部分CAD中绘制图形的功能，我们可以通过默认的命令去调用这些函数，也可以选择导出调用，具体操作可参考[命令行教程](./1.%E5%91%BD%E4%BB%A4%E8%A1%8C.md)。
