...
Proxy entity type is a special type of entity whose geometric shape and attribute information are stored in external sources, such as external files or databases.
...
```ts
//ProxyEntity is a valid proxy entity
...
Console.log ("original class name", originalName)
```
