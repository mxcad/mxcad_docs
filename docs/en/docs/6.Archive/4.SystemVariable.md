...
In CAD, line width can be regarded as the width attribute of the contour lines of a graphic object, used to control the thickness of the lines when drawing or displaying the object. After enabling line width display, users can intuitively distinguish the differences in line width among different objects, thereby more clearly expressing the hierarchical structure and design intent of the graphics, similar to using different thick and thin ink lines on a drawing to distinguish the main structure from auxiliary annotations. Line width display also helps to check the rationality of the line width settings in the drawing, serving as a reference for visual verification before printing output.

We can control whether to display the line width by obtaining the value of the system variable `LWDISPLAY`, and update the display status by using the corresponding method.

| variable name | Variable value | description |
|------|------|------|
| LWDISPLAY | 1 | Enable the line width mode |
| LWDISPLAY | 0 | Disable the line width mode |

```ts
import { MxCpp } from "mxcad"

const mxcad = MxCpp.getCurrentMxCAD();
// Obtain the option to display line width
const res = mxcad.getSysVarLong("LWDISPLAY");
// 1: Display line width, 0: Do not display line width
console.log("Whether to display line width",res);

// Display line width
mxcad.setSysVarLong("LWDISPLAY", 1);
mxcad.showLineWeight(true);
mxcad.updateDisplay();

// Do not display line width
mxcad.setSysVarLong("LWDISPLAY", 0);
mxcad.showLineWeight(false);
mxcad.updateDisplay();
```
