...
`mxweb file` is a specific file type that mxcad supports to open, and the traditional CAD drawing file format can be converted to `mxweb file` through the cloud map development kit, the specific operation can be referred to [Drawing conversion](../1.开始/3.图纸转换.md)。

### Open the mxweb file

The first parameter passed to the mxcad instance object is the network path of the mxweb file, that is, the corresponding `mxweb file` can be accessed directly through the Internet. If we want to open the `mxweb file` directly locally, we can call the method[openWebFile()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McObject.html#openwebfile) open the local 'mxweb file' directly.
...
Click on [McObject saveFileToUrl()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McObject.html#savefiletourl) to check the details.
