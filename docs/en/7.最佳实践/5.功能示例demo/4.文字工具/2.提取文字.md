...
We according to [McObjectId.getMcDbEntity()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McObjectId.html#getmcdbentity) Get the target text entity, Through [McDbEntity.objectName](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbAlignedDimension.html#objectname) Attribute to get the entity name, because in the above steps we filter out are text entities, so we only need to determine whether it is a multi-line text class, or a single-line text class, and call the corresponding text attribute according to different text classes to get text information.
```ts
let content: string = '';
