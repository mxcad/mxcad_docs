...
We can call [MxCADSelectionSet](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADSelectionSet.html#class-mxcadselectionset)  Select the entity that you want to scale. The maximum and minimum points of the bounding box are obtained according to the selected entity, and the scaling distance ratio is set according to the obtained data.
...
We by calling [MxCADUiPrPoint](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUiPrPoint.html) take the object to determine the scaling of the object's basis points, Call [MxCADUiPrDist](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUiPrDist.html) to dynamically set or direct input zoom zoom factor of objects.
...
Since there are two ways to obtain the scale factor, one is to calculate the scale factor according to the dynamic distance between the user's moving mouse position and the base point, and the other is to input the entity's scale factor directly through the input box. We can go through [MxCADUiPrDist.getDetailedResult()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUiPrDist.html#getdetailedres) The ult method obtains the detailed reason for the interaction exit and sets the scaling factor of the entity according to the two methods respectively. After scaling, Call [McObject.updateDisplay()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McObject.html#updatedisplay) method to update the drawing shows.
