...
Below we will show how to use the mxcad plug-in to implement the curve breaking function in CAD drawings. In this function, users can cut a complete curve into two or more parts by breaking points, and these cut parts will become independent curves. Among them, curve-like entities in mxcad include: [McDbPolyline](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbPolyline.html) Section of the line, [McDbArc](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbArc.html) Circular arc, [McDbCircle](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbCircle.html) Round, [McDbEllipse](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbEllipse.html) Oval/arc, [McDbLine](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbLine.html) in a straight line. The interrupt function can help users to measure, delete and adjust the length of the target curve in the drawing efficiently.

## Function implementation

1. Select the target curve

We can set the filter by [MxCADResbuf](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADResbuf.html#class-mxcadresbuf), Call again [MxCADUiPrEntity](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUiPrEntity.html) for the user to select entity, Will filter the user can choose only [McDbCurve](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbCurve.html) curve type of entity is convenient we interrupted by subsequent operations. To make it easier for us to visualize which entity is selected, We can be the selected entities through [McDbCurve.highlight()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbCurve.html) methods to set is highlighted.
...
We can [MxCADUiPrPoint](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUiPrPoint.html) in the above steps on the target curve take operation, These selected points are the breakpoints of the target curve. During the drawing process, we can dynamically draw a circle with the breakpoint as the center of the circle to facilitate us to observe the specific location of the breakpoint.
...
After completing the above steps, We will target curve according to these breakpoints calls [McDbCurve.splitCurves()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbCurve.html#splitcurves) Method March line cutting, and then we can get the interrupted curve.
...
We can call [McDbCurve.IntersectWith()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbCurve.html#intersectwith) Methods The intersections between curves are obtained, and these intersections are used as the breaking points of curves to break curves. In the following example, we use the intersection points between curves as breakpoints to cut the curve and draw a certain length of the fracture for easy observation. Users can make corresponding adjustments according to their actual needs.
