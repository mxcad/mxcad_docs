...
We can use [MxCADUiPrPoint](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUiPrPoint.html) take the object selected on the drawing need to measure the target object.
...
We can choose using entity object [MxCADUiPrEntity()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUiPrEntity.html) According to the coordinates of the user's mouse click to get the corresponding entity, where we need to select only the line object, so, We'll call [MxCADResbuf()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADResbuf.html) to select entity object set filter to filter out the target entity.
...
In mxcad linear object corresponding to the entity class to [McDbLine](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbLine.html), This class provides properties or methods to get or set information about a line, which we can choose to call according to our functional requirements. In the function of measuring the distance from the point to the line, We can call [McDbLine.getClosestPointTo()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbLine.html#getclosestpointto)  Method obtains the vertical point from the target point to the target line object, and the distance from the vertical point to the target point object is the distance from the point to the line. Finally, we call again [mxcad.drawDimAligned()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McObject.html#drawdimaligned) Draw alignment marks.
