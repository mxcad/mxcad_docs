...
In order to facilitate later management and annotation modification, We can through inheritance [McDbCustomEntity](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbCustomEntity.html) Custom entity classes to extend the implementation of custom continuous measurement classes. Since continuous measurement is to calculate the combined length of an arc and a straight line, We can be a continuous line and arc integrated into [McDbPolyline](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbPolyline.html) section of the line. Among them, we can draw the arc by setting the bulge bulge of the multi-segment line.

Then, we can use [McDbText](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbText.html) measurement information text object, the object combination the length of the label information on the page.
...
```
* we call [MxCADUiPrPoint](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUiPrPoint.html#class-mxcaduiprpoint) Continuous take take objects, and can be set by [MxCADUiPrPoint.setKeyWords()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUiPrPoint.html#setkeywords) key word list to control operation process backs up a capture point or end point object capture. Among them, In the process of capturing the arc we need to call [MxCADUtility.findEntAtPoint()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUtilityClass.html#findentatpoint) method to determine when we take the choice of entity object and call the [MxCADResbuf](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADResbuf.html) to filter out the target curve, Then call [MxCADUtility.calcBulge()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUtilityClass.html#calcbulge) Method The convexity of the arc was obtained. It is worth noting that when capturing an arc, we need to capture a certain end point of the arc before capturing the entire arc, so as to ensure that the line segment we capture is a continuous line segment.
