...
In order to facilitate later management and annotation modification, We can through inheritance [McDbCustomEntity](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbCustomEntity.html) custom entity class to extend round annotation classes. Among them, the round object in the mxcad corresponding entities such as [McDbCircle](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbCircle.html), This class provides properties or methods to get or set information about the circle, which we can choose to call according to our functional requirements. In the circle measurement function, we need to obtain the radius, area and circumference of the circle object, Radius, so we can call [McDbCircle](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbCircle.html#radius) properties for circle radius, [McDbCircle.getArea()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbCircle.html#getarea) method or directly to obtain a circle area, And [McDbCircle.getLength()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbCircle.html#getlength) method to get the circumference.

Then, we can use [McDbMText](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.McDbMText.html) measurement information text object, to draw round label information on the page.
...
We can choose using entity object [MxCADUiPrEntity()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUiPrEntity.html) According to the coordinates of the user's mouse click to get the corresponding entity, where we need to select only the circle object, so, We'll call [MxCADResbuf()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADResbuf.html) to select entity object set filter to filter out the target entity.
...
We can use [MxCADUiPrPoint](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUiPrPoint.html) take the object interaction take in the page. In the process of taking points, We can go through [MxCADUiPrPoint.setUserDraw()](https://www.mxdraw3d.com/mxcad_docs/api/classes/2d.MxCADUiPrPoint.html#setuserdraw)  Method Dynamically drawing annotation objects, so that users can observe the position changes of annotation objects more intuitively.
