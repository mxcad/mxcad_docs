[mxcad_2d API documentation] (../README. md)/[2d] (../modules/2d. md)/MxCADUiPrString

# Class: MxCADUiPrString

[2d](../modules/2d.md).MxCADUiPrString

MxCADUiPrString is a prompt class used for interactive input strings.

**`Description`**

This class is used to allow users to input text parameters such as layer name, annotation text, block name, file name, etc. in CAD commands.
It is suitable for interactive editing processes that require dynamic retrieval of string parameters.
Usage:
1. New MxCADUiPrString() creates a string input device;
2. Call setMessage () to set prompt information;
3. Call go() and wait for user input;
4. Use value() to retrieve the final string input by the user.
...
GetStr. setMessage ("Please enter layer name");
