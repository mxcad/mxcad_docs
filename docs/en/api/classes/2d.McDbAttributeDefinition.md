...
Representing block attribute definition entities, used to define attribute labels and default text in block definitions.

**`Description`**

McDbAttributeDefinition is a text attribute entity used for block definition, which supports setting labels, displaying text, position, alignment, and visibility.
It is usually saved along with the block definition, and after inserting the block, it can prompt the user to enter the attribute value and display the attribute text in the graphic.
Instructions:
1. Create an instance of 'McDbAttributeDefinition' and set attributes such as' tag ',' textString ',' position ', etc.
2. Append the attribute definition to the block definition object.
After inserting the block reference, the system will prompt the user to fill in the attribute value and display the attribute text according to the definition.
...
AttribDef.textString="default attribute value";
