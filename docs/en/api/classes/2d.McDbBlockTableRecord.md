...
McDbBlockTableRecord is a block table record object used to manage entity collections, base points, and block properties in block definitions.

**`Description`**

This class represents a block definition record in CAD, mainly used to maintain the entity content, block base points, and block names inside the block.
It can accomplish:
-Add entity objects to the block;
-Obtain the object IDs of all entities in the block;
-Set the base point, bounding box, and name of the block;
-As the source of block reference definition, it can be displayed and inserted through McDbBlockReference in the future.

Summary of usage:
1. Create an instance of McDbBlockTableRecord;
2. Call appendAcDbEntity () to add the entity to the block;
3. Set the base point and name of the block through attributes such as origin and name;
4. Add the block record to the block table and create a block reference display using McDbBlockReference.
...
//Select the block object and modify its color.
...
- [getGripEditBasePointsIndex](2d.McDbBlockTableRecord.md#getgripeditbasepointsindex)
