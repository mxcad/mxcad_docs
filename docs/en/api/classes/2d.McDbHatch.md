...
Represents a hatch entity used to fill enclosed areas with solid colors or patterns.

**`Description`**

McDbHatch provides region filling functionality based on pattern definition, color control, and boundary loop checking.   
It can be created from closed areas, configured with custom pattern parameters, and drawn in the form of filled entities. *
Usage:
1. Create or obtain a McDbHatch instance for enclosing fillable areas.
2. Clear or define the fill pattern and set the desired pattern or color.
3. Draw filled entities or save their entities as part of the drawing database.
...
| Name | Type | Description |
| :------ | :------ | :------ |
|IsDouble | boolean | Double or not|
...
Recalculate the pattern definition data based on the current pattern scale, pattern definition, and angle data.
After calculation, the pattern scale and angle attributes have been included in the pattern definition data. The pattern scale and angle attributes have been reset to 0, and the pattern scale has been reset to 1
