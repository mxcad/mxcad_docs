...
Interactive distance input class, used to obtain distance values between two points.

**`Description`**

`MxCADUiPrList is an interactive control based on MxCADUiPrBase, used to prompt users to determine a distance value through two-point input.
It supports setting prompt information, dynamically dragging base points, dynamically drawing callbacks, and obtaining final distance values and detailed exit reasons.
Commonly used for annotation, measurement, or interactive scenarios that require users to determine length.
Instructions:
1. Create an instance of MxCADUiPrList and set prompt information through setMessage;
2. (Optional) Use ` setBasePt `/` setUserDraw ` to set dynamic drag base points or dynamic drawing effects to enhance the user experience;
3. Call 'go()' to wait for user input, then use 'value()' to obtain the distance result and use 'getDetailedResult()' to determine the reason for exiting.
...
GetDist.setMessage ('\ nSpecify the first and second points to determine the distance:');
//Optional: Set dynamic drawing
  getDist.setUserDraw((pt, worldDraw) => {
    worldDraw.setColor(0x00ff00);
//Assuming to draw a temporary marker here
  });

  const val = await getDist.go();
if (val == null) return; //User cancellation or other reasons
...
- [getLastInputPoint](2d.MxCADUiPrDist.md#getlastinputpoint)
...
- [setOsModeExParam](2d.MxCADUiPrDist.md#setosmodeexparam)
...
| Name | Type | Description |
| :------ | :------ | :------ |
|` useIt ` | ` boolean ` | Whether to enable base point for dynamic following/dynamic drawing defaults to false|
