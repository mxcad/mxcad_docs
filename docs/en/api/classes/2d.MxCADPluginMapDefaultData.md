...
MxCADPluginMapDefaultData represents the default configuration for aligning maps with CAD drawings.

**`Description`**

This class is used to describe the alignment relationship between maps and CAD drawings, including CAD origin, map origin, unit conversion, default open files, tile layers, and other information.
It is commonly used in plugins or map integration scenes to ensure that CAD content is correctly aligned with the map canvas.
Summary of usage:
1. Construct an instance of 'MxCADPluginMapDefaultData';
2. Set alignment parameters such as' cadOrigin ',' mapOrigin ',' meterNiCADUnits', etc;
3. Pass it as part of the plugin configuration to the host.
