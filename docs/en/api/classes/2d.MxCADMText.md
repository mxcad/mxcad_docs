...
MxCAD is a multi line text entity class used for rendering, editing, and managing rich text content in CAD.

**`Description`**

MxCADMText is a custom multi line text entity suitable for displaying rich text, auto wrap, line height control, text alignment, and multi paragraph layout in MxCAD scenes.
This class is capable of completing:
-Create and manage data and styles for multiline text objects;
-Arrange the layout according to the specified width, row height, and alignment method;
-Support dynamic updates of attributes such as angle, position, maximum width, etc;
-During the worldDraw stage, recursively render the text content into the CAD interface by element;
-Handle grip editing, coordinate transformation, and custom transformation logic.

Usage:
1. Instantiate MxCADMText and set properties such as data, position, maxWidth, lineHeightRatio, etc;
2. Call properties such as angle, textAlignment, isWordWrap to update rendering status when needed;
3. Use worldDraw() to draw the text content into the current graphics environment;
4. Call customizeTransformBy() or transformBy() to complete the matrix transformation;
5. Draw the text entity to the CAD interface using MxCpp. mxcad. rawEntity ().

Summary: MxCADMText is a rich text custom entity in MxCAD scenes, typically used for text scenes that require complex multiline layout, editing interaction, and custom rendering.
...
- [customizeTransformBy](2d.MxCADMText.md#customizetransformby)
...
- [getGripEditBasePointsIndex](2d.MxCADMText.md#getgripeditbasepointsindex)
...
▪ `Static` **toMText**: (`customEntity`: [`MxCADMText`](2d.MxCADMText.md), `mtext?`: [`McDbMText`](2d.McDbMText.md)) => ([`McDbEntity`](2d.McDbEntity.md) \| [`McDbCustomEntity`](2d.McDbCustomEntity.md))[] = `MultilineTextConverter.toMText`

#### Type declaration

▸ (`customEntity`, `mtext?`): ([`McDbEntity`](2d.McDbEntity.md) \| [`McDbCustomEntity`](2d.McDbCustomEntity.md))[]
...
### customizeTransformBy

▸ **customizeTransformBy**(`mat`): `boolean`

Custom transformation matrix function
...
| Name | Type |
| :------ | :------ |
| `_met` | [`McGeMatrix3d`](2d.McGeMatrix3d.md) |
