[mxcad_2d API documentation] (../README. md)/[2d] (../modules/2d. md)/McDbPoint

# Class: McDbPoint

[2d](../modules/2d.md).McDbPoint

McDbPoint represents a single point entity in a drawing.

**`Description`**

McDbPoint contains a point object with clear world coordinates and appearance control.   
Support setting positions and use the point as a CAD solid that can be drawn for dimensioning, snap points, or simple graphic references. *
Usage:
1. Create a new McDbPoint.
2. Set its position and optional display properties, such as trueColor.
3. Draw the point using the current MxCAD context.
...
Const point=new McDbPoint()//Construct a point object
   const color = new McCmColor(0,255,0)
Point.trueColor=color//Set the color of the point object
Point. setPosition (0,0)//Set the position of the point object
MxCpp. getCurrentMxCAD(). drawEntity (point)//Draw point object
```
...
- [thickness](2d.McDbPoint.md#thickness)
...
- [getGripEditBasePointsIndex](2d.McDbPoint.md#getgripeditbasepointsindex)
