...
McDbBlockTable is a block table in the database, responsible for managing all block definitions in the current drawing.

**`Description`**

This class is used to maintain and access the set of block definitions in drawings, including traversal, addition, search, and determining whether block records exist.
It can accomplish:
-Obtain all block record IDs;
-Add a new named block definition;
-Search for block records by name;
-Determine whether the block exists;
-Provide entry points for subsequent block insertions and block references.

Usage:
1. Retrieve the block table ` MxCpp. getCurrentMxCAD(). getBlockTable() ` from the database;
2. Call getAllRecordId() to view all block table record objects;
3. Use add()/get()/has() to manage block definitions;
4. Retrieve McDbBlockTableRecord from the returned McObjectid and continue editing the block content.

Summary: In practical development, McDbBlockTable is the core entry point for block management, suitable for creating and retrieving block definitions, supporting block insertion and dynamic block development.
